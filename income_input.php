...
if ($conn->connect_error) {
    ...
    // For this example, we'll try to use SweetAlert2.
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (isset($_POST['confirm_submission']) && $_POST['confirm_submission'] === 'true') {
            ...
            if (empty($amount) || !is_numeric($amount) || $amount <= 0) {
                ...
                // Get current timestamp for income_date (fool-proof date)
                $income_date = date('Y-m-d H:i:s'); // Changed to include time for more precision

                // Prepare an insert statement
                $sql = "INSERT INTO income (user_id, amount, description, income_date) VALUES (?, ?, ?, ?)";

                if ($stmt = $conn->prepare($sql)) {
                    $stmt->bind_param('idss', $user_id, $amount, $description, $income_date);
                    if ($stmt->execute()) {
                        // Redirect to dashboard with success message
                        header('Location: dashboard.php?swal_message=' . urlencode('Income recorded successfully!') . '&swal_type=success');
                        exit();
                    } else {
                        $swal_message = 'Error recording income: ' . $stmt->error;
                        $swal_type = 'error';
                    }
                    $stmt->close();
                } else {
            ...
            // Initial submission, show summary for confirmation
            if (empty($amount) || !is_numeric($amount) || $amount <= 0) {
                $swal_message = 'Please enter a valid positive amount.';
                $swal_type = 'error';
            } else {
...
<head>
    ...
    <title>Record Income - Executive Dashboard</title>
    <!-- Tailwind CSS CDN -->
    ...
    <style>
        ...
        body {
            ...
            align-items: center;
            background-image: linear-gradient(to right bottom, #d1fae5, #a7f3d0); /* Greenish gradient */
        }
        .container {
            max-width: 1200px;
            margin: 0 auto;
...
<body class="bg-gray-100">
    <div class="input-card bg-white p-8 rounded-xl shadow-2xl w-full max-w-md transform transition duration-300 ease-in-out hover:scale-105">
        <h2 class="text-3xl font-bold text-center text-gray-800 mb-6">Record New Income</h2>
        ...
        <?php if (isset($show_confirmation) && $show_confirmation): ?>
            <div class="bg-blue-50 p-6 rounded-lg mb-6 text-center shadow-inner">
                <p class="text-lg text-blue-800 mb-4">Please confirm your income entry:</p>
                <p class="text-xl font-bold text-green-700 mb-2">Amount: <?php echo htmlspecialchars($currency); ?><?php echo number_format($amount, 2); ?></p>
                <p class="text-md text-gray-700">Description: <?php echo htmlspecialchars($description ?: 'N/A'); ?></p>
            </div>
            <form action="income_input.php" method="POST" class="space-y-4">
                ...
                <button type="submit"
                        class="w-full bg-green-600 text-white py-3 rounded-lg hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-opacity-75 font-semibold transition duration-200 shadow-lg transform hover:scale-105">
                    Confirm & Record Income
                </button>
                <button type="button" onclick="window.location.href='income_input.php'"
        ...
        <?php else: ?>
            <form action="income_input.php" method="POST" class="space-y-6">
                <div>
